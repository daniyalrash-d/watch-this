...
export async function POST(request: Request) {
  try {
    ...
    let prompt = ''
    if (title && genre) {
      prompt = `Suggest top 10 ${type}s similar to the "${title}" in the ${genre} genre. Strictly Provide titles only, no other texts in response.`
    } else if (title) {
      prompt = `Suggest top 10 ${type}s similar to the "${title}". Strictly Provide titles only, no other texts in response.`
    } else if (genre) {
